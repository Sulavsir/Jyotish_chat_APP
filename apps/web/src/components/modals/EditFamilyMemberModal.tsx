 ...
 * Form to edit an existing family/friend profile (birth details).
 */
...
export interface EditFamilyMemberModalProps {
  isOpen: boolean;
  onClose: () => void;
...
function profileToFormValues(profile: ClientProfile): ProfileFormValues {
  const dob = profile.dateOfBirth
    ? new Date(profile.dateOfBirth as string | Date).toISOString().split('T')[0]
    : undefined;
  return {
    name: profile.name,
...
function preparePayload(values: ProfileFormValues) {
  const dateOfBirth =
...
export function EditFamilyMemberModal({
  ...
  const updateMutation = useMutation({
    mutationFn: (values: ProfileFormValues) =>
      clientProfileService.update(profile!.id, preparePayload(values)),
    onSuccess: (data) => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && handleClose()}>
      <DialogContent className="max-w-md border border-white/10 bg-slate-900/95 backdrop-blur-md text-white shadow-xl shadow-black/20">
        ...
        >
          <div>
            <Label htmlFor="edit-profile-name" className="text-gray-300">
              Name
            </Label>
            <Input
          ...
          <div>
            <Label htmlFor="edit-profile-relationship" className="text-gray-300">
              Relationship
            </Label>
            <Input
