...
export const OnlineUsers: React.FC<OnlineUsersProps> = ({ title, maxUsers = 10 }) => {
  ...
  const loadUsers = async () => {
    try {
      setIsLoading(true);
      const response = await apiClient.get<User[]>('/api/v1/users/chatable');
      setUsers(response || []);
    } catch (error) {
  ...
  const onlineUsersFiltered = users.filter((user) => onlineUsers.has(user.id)).slice(0, maxUsers);
